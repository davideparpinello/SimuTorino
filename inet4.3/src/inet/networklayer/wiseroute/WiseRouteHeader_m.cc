...
EXECUTE_ON_STARTUP(
    omnetpp::cEnum *e = omnetpp::cEnum::find("inet::WiseRouteMsgType");
    if (!e) omnetpp::enums.getInstance()->add(e = new omnetpp::cEnum("inet::WiseRouteMsgType"));
    e->insert(DATA, "DATA");
    e->insert(ROUTE_FLOOD, "ROUTE_FLOOD");
)
...
inet::WiseRouteMsgType WiseRouteHeader::getHeaderKind() const
{
    return this->headerKind;
}

void WiseRouteHeader::setHeaderKind(inet::WiseRouteMsgType headerKind)
{
    handleChange();
...
int WiseRouteHeaderDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'h' && strcmp(fieldName, "headerKind") == 0) return base+5;
    ...
    if (fieldName[0] == 'i' && strcmp(fieldName, "initialSrcAddr") == 0) return base+8;
    if (fieldName[0] == 'p' && strcmp(fieldName, "payloadLengthField") == 0) return base+9;
